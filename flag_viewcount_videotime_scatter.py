...
# グラフプロット
## 再生回数と動画時間の散布図をプロット
sc = plt.scatter(y9, y1, s=50, c="lightblue", alpha=0.5, linewidths=1, edgecolors="red")
#plt.colorbar()


coef = P.polyfit(vt_seconds, y1, 1)
 
# 作成した多項式近似を表示
print("viewCount & videoTime approximate: " + 'y = ' + str(coef[0]) + ' + ' + str(coef[1]) + ' * x')

line = P.polyval(vt_seconds, coef)

approximate, = plt.plot(vt_seconds, line, color='k')
...
# ラベル名
plt.xlabel("動画時間", fontsize=10, fontname="TakaoPGothic")
...
#plt.legend([approximate], [approximate_title], bbox_to_anchor=(1.0, 1.0), prop={"family":"TakaoPGothic", 'size':10}, markerscale=1)

#画像保存
plt.savefig("flag_viewCount_videoTime_scatter_graph.png", bbox_inches="tight", pad_inches=0.0)