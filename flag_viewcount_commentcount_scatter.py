...
# グラフプロット
## 再生回数と動画時間の散布図をプロット
sc = plt.scatter(y4, y1, s=50, c="k", alpha=0.5, linewidths=1, edgecolors="r")
#plt.colorbar()


coef = P.polyfit(y4, y1, 1)
 
# 作成した多項式近似を表示
print("viewCount & commentCount approximate: " + 'y = ' + str(coef[0]) + ' + ' + str(coef[1]) + ' * x')

line = P.polyval(y4,coef)
...
# ラベル名
plt.xlabel("コメント数", fontsize=10, fontname="TakaoPGothic")
...
#画像保存
plt.savefig("flag_viewCount_commentCount_scatter_graph.png", bbox_inches="tight", pad_inches=0.0)